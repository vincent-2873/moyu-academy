...
| `brand_manager` | 品牌總監 | 0 | `/admin` (該品牌,已 enforce) | bcrypt 個人密碼 |
| `sales_manager / recruit_manager / legal_manager` | 部門主管 | 0 | `/admin` (該品牌,已 enforce) | bcrypt 個人密碼 |
| `team_leader` | 團隊長 | 1 | `/admin` (該團隊,已 enforce) | bcrypt 個人密碼 |
| `trainer / mentor` | 訓練師 / 師傅 | 0 | `/admin` (只讀,已 enforce by enforceWriteAccess) | bcrypt 個人密碼 |
| `recruiter / hr` | 招聘員 / HR | — | `/admin` (獵頭 scope) | bcrypt 個人密碼 |
| `sales_rep / sales_rookie / recruiter / legal_staff` | 員工 | 52 | **只能** `/me` | passwordless（或 LINE 一鍵登入） |
| `student` | 學員(墨宇課程學員,非員工) | (未定義) | **只能** `/me` 看自己訓練進度 | passwordless,跟 sales_rep 一樣 scope |
...
| 狀態 | 項目 | 備註 |
|---|---|---|
| ✅ Done | ~~`/api/me/*` 加 caller-ID 驗證~~ | 2026-04-30 sprint:`lib/auth.ts` HMAC cookie + 12 個 me/* endpoint 全套 `requireCallerEmail` |
| ✅ Done | ~~`/api/admin/sales-metrics` 依 `team_leader.team` / `brand_manager.brand` 過濾~~ | 2026-04-30:`lib/admin-scope.ts` `applyScopeFilter` |
| ✅ Done | ~~加 per-user admin password（或走 OAuth）~~ | bcrypt + `users.password_hash` migration;預設 `0000` 首次登入強制改 |
| ✅ Done | ~~`trainer / mentor` role 實作只讀模式~~ | 2026-04-30:`enforceWriteAccess(scope, method)` 在寫操作端點呼叫 |
| ✅ Done | ~~`/api/v3/commands` POST 檢查 owner 是否在 scope 內~~ | 2026-04-30:`ensureOwnerInScope` 已套用 |
| ✅ Done | ~~`student` role 定義~~ | 2026-04-30:角色 matrix 加入 student(只能 /me 看自己訓練進度) |
...
| ⏸️ TODO | 把 `enforceWriteAccess` 套到所有 admin write endpoint(POST/PATCH/DELETE) | 目前只在 v3/commands;其他 admin endpoint 依 trainer 沒 UI 入口先放(隱藏 button) |
