...
UNION ALL SELECT 'distinct_emails', COUNT(DISTINCT user_email)::text FROM public.sales_metrics_daily WHERE user_email IS NOT NULL
UNION ALL SELECT 'xunlian_distinct', COUNT(DISTINCT user_email)::text FROM public.sales_metrics_daily WHERE user_name LIKE '新訓-%' OR user_name LIKE '新訓 %' OR user_name LIKE '新訓:%'
...
SELECT brand,
       COUNT(*)::int AS rows,
       COUNT(DISTINCT user_email)::int AS distinct_users,
