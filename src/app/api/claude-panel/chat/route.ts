...
# 銷售方法論(顧問式 + 引導式 + 蘇格拉底)— D20 SQL 4 本書 reading module 對齊
- **GROW**(目標 Goal / 現況 Reality / 選項 Options / 意願 Will)
- **SPIN**(情境 Situation / 問題 Problem / 暗示 Implication / 需求效益 Need-Payoff)
- **黃金圈**(Why → How → What,從動機講起)
- **OKR**(Objectives / Key Results,KPI 漏斗拆解用)
...
# nSchool 開發 8 步驟(BIZ 業務開發核心,Source: content/training/sales/nschool/Categories/訓練中心/開發檢核/)
1. **破冰** — 建立親和力,IG 看到客戶關注財富密碼來電,問開放問題挖客戶在哪個投資領域有興趣
2. **信任建立** — 用第三人(你/我/他)視角說法,數據+案例+複利效應(10萬投30年到174萬)消除抗拒
3. **需求探索** — 拆解投資現狀(基本面/技術面/籌碼面)+ 痛點(被套/不知何時買賣/追高被坑)
4. **介紹nSchool** — 凱衛資訊上市櫃 5201 + 20-30 年金融軟硬體 + nStock/Cmoney 同集團 + 教育社會責任
5. **補充資訊** — 新人蒐集 2 重點(需求 + 預算)+ 起承轉合(別太早介紹我是誰,先建立信任)
6. **財經架構** — 盈虧不對稱(賺10賠10的時間成本)→ 時間複利演算滾雪球 → 基本/技術/籌碼面導入
7. **產品引導與價值說明** — 客製化 vs HAHOW 整包 + 4-5 位財經教練即時解惑 + 5-10% 學習成本論
8. **行動邀請** — 30 min Google Meet 免費試聽 + 2 選 1 漏斗(明天/後天 + 早午晚)+ 加 LINE @5201nschool
      ...
      try {
        const embRes = await fetch("https://api.openai.com/v1/embeddings", {
          ...
          body: JSON.stringify({
            ...
            input: lastUserMsg.content.slice(0, 4000),  // OpenAI 8192 tok / 中文 1 char≈1 tok 留 headroom
          }),
        });
        if (embRes.ok) {
