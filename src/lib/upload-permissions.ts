 ...
 *
 * Vincent 反饋:只有以下 4 種 role 可以上傳到 RAG:
 *   - super_admin(超級管理員)
 *   - sales_manager(業務管理)
 *   - legal_manager(法務管理)
 *   - recruit_manager(招聘/人資管理)
 *
 * 適用所有上傳模式:admin / staff / self(任何 source 都要 role check)
 *
 * 設計理由:
 *   - 員工亂上傳會污染 RAG 池
 *   - 4 個 role 就是各 pillar 的負責人 + super_admin
 *   - hr_manager 跟 recruit_manager 為同一概念(現 schema 用 recruit_manager)
 */
